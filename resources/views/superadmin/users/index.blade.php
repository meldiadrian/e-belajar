...
@section('page_title', 'Kelola Pengguna & Peran')
...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex border-b border-slate-200 text-xs font-bold gap-4">
            ...
            <a href="{{ route('superadmin.users.index', ['status' => 'trashed']) }}"
                ...
                <span
                    class="px-2 py-0.5 rounded-full text-[10px] {{ ($status ?? 'active') === 'trashed' ? 'bg-amber-100 text-amber-800' : 'bg-slate-100 text-slate-600' }}">{{ $trashedCount }}</span>
            </a>
        </div>

        <!-- Filter & Search Bar -->
        <form method="GET" action="{{ route('superadmin.users.index') }}"
            class="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex flex-wrap gap-4 items-center justify-between">
                        ...
                        @forelse($users as $user)
                            <tr class="hover:bg-slate-50/50">
                                    ...
                                    @else
                                        <form action="{{ route('superadmin.users.update', $user->id) }}" method="POST"
                                            ...
                                            <select name="role" onchange="this.form.submit()"
                                                class="px-2 py-1 rounded-lg text-xs font-bold border border-slate-200 {{ $user->isSuperAdmin() ? 'bg-amber-50 text-amber-900 font-bold' : ($user->isAdmin() ? 'bg-teal-50 text-teal-900' : 'bg-slate-100 text-slate-800') }}">
                                ...
                                <td class="py-3.5 px-4 text-slate-700">
                                    ...
                                    <div class="text-[11px] text-amber-600 font-semibold">{{ $user->certificates_count }}
                                        Sertifikat</div>
                                </td>
                                <td class="py-3.5 px-4 text-right">
                                    @if($user->trashed())
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            <form action="{{ route('superadmin.users.force-delete', $user->id) }}" method="POST"
                                                class="inline"
                                                onsubmit="return confirm('Hapus permanen akun {{ $user->name }}? Tindakan ini tidak dapat dibatalkan.')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="px-2.5 py-1 rounded-lg bg-red-50 hover:bg-red-100 text-red-600 font-bold text-[10px] transition-colors"
    ...
    <!-- Modal: Add User -->
    <div id="modalAddUser"
        class="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 {{ $errors->any() ? '' : 'hidden' }}">
