...
@section('page_title', 'Tambah Pertanyaan Umum')
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-white p-8 rounded-3xl border border-slate-200 shadow-xs space-y-6">
        <div>
            <h2 class="text-lg font-black text-slate-900">Formulir Tambah FAQ</h2>
            <p class="text-xs text-slate-500 mt-0.5">Isi pertanyaan dan jawaban yang informatif untuk membantu peserta portal.</p>
        </div>
        ...
        <form action="{{ route('admin.faqs.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="question" id="question" value="{{ old('question') }}" required placeholder="Contoh: Bagaimana cara mengunduh sertifikat digital?" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:border-emerald-600 focus:outline-hidden">
                @error('question')
            ...
            <div>
                ...
                <textarea name="answer" id="answer" rows="5" required placeholder="Tuliskan uraian jawaban secara jelas dan lengkap..." class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:border-emerald-600 focus:outline-hidden leading-relaxed">{{ old('answer') }}</textarea>
                @error('answer')
                    <span class="text-xs text-red-600 mt-1 block">{{ $message }}</span>
                @enderror
            </div>
                ...
                <div>
                    ...
                    <select name="faq_category_id" id="faq_category_id" required class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:border-emerald-600 focus:outline-hidden bg-white">
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}" {{ (old('faq_category_id') == $cat->id || (empty(old('faq_category_id')) && $loop->first)) ? 'selected' : '' }}>
                                {{ $cat->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="pt-2">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_published" value="1" {{ old('is_published', '1') ? 'checked' : '' }} class="w-4 h-4 rounded text-emerald-700 focus:ring-emerald-500 border-slate-300">
                    <div>
                        <span class="text-sm font-bold text-slate-800">Publikasikan Langsung</span>
                        <p class="text-xs text-slate-500">Jika dicentang, pertanyaan ini akan langsung tampil di halaman publik FAQ.</p>
                    </div>
                </label>
            </div>
