...
@section('page_title', 'Kelola Pertanyaan Umum (FAQ)')
...
<div class="space-y-6">
    ...
    <div class="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h2 class="text-xl font-black text-slate-800">Daftar Pertanyaan Umum (FAQ)</h2>
                <p class="text-xs text-slate-500 mt-1">Kelola daftar tanya jawab yang tampil pada portal publik E-Belajar secara dinamis.</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('faqs.index') }}" target="_blank" class="px-4 py-2.5 rounded-xl border border-slate-300 text-slate-700 hover:bg-slate-50 font-bold text-xs transition-colors flex items-center gap-1.5">
    ...
    <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs">
        <form method="GET" action="{{ route('admin.faqs.index') }}" class="flex flex-col sm:flex-row gap-3">
            ...
            @if($search || $category)
                <a href="{{ route('admin.faqs.index') }}" class="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold text-xs transition-colors flex items-center shrink-0">
                    Reset
                </a>
            @endif
        </form>
            ...
            <table class="w-full text-left text-xs">
                <thead class="bg-slate-50 border-b border-slate-200 text-slate-600 font-bold uppercase tracking-wider text-[11px]">
                    <tr>
                        ...
                        <th class="px-5 py-3.5">Pertanyaan & Jawaban</th>
                        ...
                        <tr class="hover:bg-slate-50/80 transition-colors">
                            ...
                            <td class="px-5 py-4 space-y-1">
                                ...
                                <div class="text-slate-500 line-clamp-2 leading-relaxed">
                                    {{ Str::limit(strip_tags($faq->answer), 160) }}
                                </div>
                            </td>
                            <td class="px-5 py-4">
                                <span class="px-2.5 py-1 rounded-full bg-slate-100 font-bold text-slate-700">
                                    {{ $faq->category }}
                                </span>
                            </td>
                            ...
                            <td class="px-5 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('admin.faqs.edit', $faq) }}" class="p-2 rounded-lg bg-amber-50 hover:bg-amber-100 text-amber-700 font-bold transition-colors" title="Edit FAQ">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                        </svg>
                                    </a>
                                    <form action="{{ route('admin.faqs.destroy', $faq) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pertanyaan ini?')" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-2 rounded-lg bg-red-50 hover:bg-red-100 text-red-600 font-bold transition-colors" title="Hapus FAQ">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="py-12 text-center text-slate-400">
                                Belum ada data FAQ yang dibuat. Silakan klik tombol "Tambah FAQ Baru".
                            </td>
                        </tr>
                    @endforelse
                </tbody>
